...
def df_sheriffs():
    ...
    return df[df[common.PEOPLE_COL].notnull() & df[common.PEOPLE_COL].str.contains('sheriff')]
...
def plt_sheriffs_counties_month_heatmap(save=False, file_name='plt_sheriffs_counties_month_heatmap.png',
    ...
    matrix = pd.DataFrame(np.zeros(shape=(len(months), len(counties))), columns=months, index=counties)
